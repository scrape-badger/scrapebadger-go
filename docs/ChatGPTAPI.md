...
> interface{} ChatgptAskChatgptAQuestion(ctx).Prompt(prompt).Country(country).WebSearch(webSearch).Execute()
...
func main() {
	...
	resp, r, err := apiClient.ChatGPTAPI.ChatgptAskChatgptAQuestion(context.Background()).Prompt(prompt).Country(country).WebSearch(webSearch).Execute()
	if err != nil {
