...
type ApiVintedSearchByImageRequest struct {
	ctx context.Context
	ApiService *VintedAPIService
	vintedImageSearchRequest *VintedImageSearchRequest
}

func (r ApiVintedSearchByImageRequest) VintedImageSearchRequest(vintedImageSearchRequest VintedImageSearchRequest) ApiVintedSearchByImageRequest {
	r.vintedImageSearchRequest = &vintedImageSearchRequest
	return r
}
...
market envelope. Visual ranking. Each item carries `similarity_score` on the calls where Vinted returns
a ranking, and null on the ones where it does not -- a null says nothing about the item. The score sits on
an unbounded scale that Vinted changes without notice (0-1 in Sep 2026, ~36-44 since): compare it only with
the other items in the SAME response, never to a fixed cut-off. Resend the same image
and pagination time for subsequent pages. Structured brand data may be null.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiVintedSearchByImageRequest
*/
func (a *VintedAPIService) VintedSearchByImage(ctx context.Context) ApiVintedSearchByImageRequest {
	return ApiVintedSearchByImageRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *VintedAPIService) VintedSearchByImageExecute(r ApiVintedSearchByImageRequest) (*SearchResponse, *http.Response, error) {
	...
	localVarPath := localBasePath + "/v1/vinted/search_by_image"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.vintedImageSearchRequest == nil {
		return localVarReturnValue, nil, reportError("vintedImageSearchRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.vintedImageSearchRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
